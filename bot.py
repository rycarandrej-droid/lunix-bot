...
# ==== СЛОВА ТРЕВОГИ И СТРАХА (ночью — поддержка) ====
FEAR_WORDS = [
...
def build_reply(text, mode, history, name, mood, last_action):
    ...
    if is_night and mode == "good":
        # Если страшно/тревожно/плохо — поддержка!
        if has_fear(text):
    ...
    # ==== «СПОКОЙНОЙ НОЧИ» — финальный ответ ====
    ...
    # ==== НОЧЬ — мягко про сон (только 10%, если не тревога) ====
