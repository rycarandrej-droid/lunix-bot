...
# Режим: True — бот отвечает сам + логи тебе. False — бот молчит.
BOT_AUTOREPLY = True
...
def log_to_admin(message, is_group=False, is_media=False):
    try:
        if message.from_user.id == ADMIN_ID:
            return
        chat_type = "👥 Группа" if is_group else "💬 Личка"
        username = message.from_user.username or "без_username"
        name = message.from_user.first_name or "?"
        user_id = message.from_user.id
        chat_id = message.chat.id
        chat_title = message.chat.title if is_group and message.chat.title else "личка"
        ...
        log = (
            f"📩 {chat_type}: {chat_title}\n"
            f"👤 {name} (@{username})\n"
            f"🆔 user_id: {user_id}\n"
            f"💬 chat_id: {chat_id}\n"
            f"📝 {text}\n\n"
            f"↩️ В чат: /reply {chat_id} текст\n"
            f"↩️ В личку: /dm {user_id} текст"
        )
        bot.send_message(ADMIN_ID, log)
    except Exception as e:
        print(f"log_to_admin error: {e}")
...
def cmd_reply(m):
    ...
    try:
        ...
        if len(parts) < 3:
            bot.send_message(m.chat.id, "Формат: /reply chat_id текст")
            return
        chat_id = int(parts[1])
        text = parts[2]
        bot.send_message(chat_id, text)
        bot.send_message(m.chat.id, f"✅ Отправлено в {chat_id}")
    except Exception as e:
...
def cmd_dm(m):
    ...
    try:
        parts = m.text.split(maxsplit=2)
        if len(parts) < 3:
            bot.send_message(m.chat.id, "Формат: /dm user_id текст")
            return
        user_id = int(parts[1])
        text = parts[2]
        bot.send_message(user_id, text)
        bot.send_message(m.chat.id, f"✅ Отправлено юзеру {user_id}")
    except Exception as e:
        bot.send_message(m.chat.id, f"❌ Ошибка: {e}")
...
def cmd_autoreply(m):
    if m.from_user.id != ADMIN_ID:
        return
    global BOT_AUTOREPLY
    BOT_AUTOREPLY = not BOT_AUTOREPLY
    status = "✅ ВКЛ" if BOT_AUTOREPLY else "❌ ВЫКЛ (бот молчит, только ты отвечаешь)"
    bot.send_message(m.chat.id, f"Автоответ бота: {status}")
